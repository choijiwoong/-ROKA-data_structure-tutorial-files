...
n = 100    # 전체 동전 개수
print(find_fakecoin_(0, n - 1))


#저울질의 횟수 처음의 경우 저울질이 최대 n-1(정확히는n-2)번 필요하므로 O(n), 둘번의 경우 O(logn).
#만약 동전을 세그룹으로 나누면 O(log3의n)으로도 가짜동전 찾기 가능(계산복잡도는 같음)
